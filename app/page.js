    ...
    >
      ...
      >
        <div className="flex flex-wrap justify-center items-center h-screen w-screen">
          <div className="uppercase text-center">Andy Shimmin</div>
          <ul className="w-full flex flex-wrap justify-between items-center p-9">
            ...
            {navData.map((item, i) => (
              <Link href={item.link}>
                <li
                  key={item.i}
                  className="uppercase font-bold text-opacity-50 hover:text-[#333341] hover:/public/fonts/Al-Khoud-Regular.otf md:text-nav"
                >
                  {item.title}
                </li>
              </Link>
          ...
          <div className="text-center">Interaction Designer & Ceramicist</div>
        </div>
      </div>
    </div>
  );
